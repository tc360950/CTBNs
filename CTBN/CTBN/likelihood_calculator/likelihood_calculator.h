...
private:
	...
	void convert_to_predictive_vector(const State &state, size_t node, std::vector<bool> &result_placeholder) const {
		if (!ADD_INTERACTIONS) {
	...
	Real_t get_intensity(const std::vector<Real_t> &beta, const std::vector<bool> &predictive) const {
		Real_t result = 0;
	...
	Real_t calculate_likelihood(const std::vector<Real_t> &beta, size_t node, bool past_node_value) const {
		...
		for (auto &transition_count : transition_repository.fetch_node_transitions(node).get_transition_counts()) {
			...
			if (state.get_node_value(node) == past_node_value) {
				convert_to_predictive_vector(state, node, predictive_vector);
