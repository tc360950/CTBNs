#ifndef CORRELATED_MODEL_NO_INTERACTIONS_H
#define CORRELATED_MODEL_NO_INTERACTIONS_H
#include <random>
#include <utility>
...
template <class Real_t> class CorrelatedModelNoInteractions {
private:
		...
		if (TEST) {
			for (size_t i = 0; i < skeleton.size() - 1; i++) {
				if (skeleton[i].second >= skeleton[i + 1].second) {
					logTest<CorrelatedModelNoInteractions>("EmptyModel: Jump times are not increasing!");
				}
				...
				if (changing_count != 1) {
					logTest<CorrelatedModelNoInteractions>("More or less than one node has been changed in a jump!");
				}
			}
			logTest<CorrelatedModelNoInteractions>("Skeleton is ok!");
		}
		return skeleton;
	}
	...
	TransitionRepository<Real_t> convert_skeleton_to_transition_repository(const std::vector<std::pair<State, Real_t>> &skeleton, const Real_t t_max) const {
		...
		if (TEST) {
			if (all_states.size() > skeleton.size()) {
				logTest<CorrelatedModelNoInteractions>("There are more states in <all_states> than in the skeleton!");
			}
			std::unordered_set<State, StateHash> all_states_set(all_states.begin(), all_states.end());
			if (all_states.size() != all_states_set.size()) {
				logTest<CorrelatedModelNoInteractions>("There are duplicate states in <all_states>!");
			}
			...
			if (std::abs(total_transition_count - skeleton.size() + 1.0) > 0.5) {
				logTest<CorrelatedModelNoInteractions>("There are more transition counts than states!");
			}
			...
			if (total_transitions != transition_to_count.size()) {
				logTest<CorrelatedModelNoInteractions>("Transition counts do not match!");
			}
			if (zero_transition_per_state > 1) {
				logTest<CorrelatedModelNoInteractions>("There are more than one state with zero transitions!");
			}
		}
