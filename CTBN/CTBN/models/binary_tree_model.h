#ifndef TREE_MODEL_NO_INTERACTIONS_H
#define TREE_MODEL_NO_INTERACTIONS_H
#include <random>
#include <utility>
		...
		if (TEST) {
			for (size_t i = 0; i < skeleton.size() - 1; i++) {
				if (skeleton[i].second >= skeleton[i + 1].second) {
					logTest<TreeModelNoInteractions>("EmptyModel: Jump times are not increasing!");
				}
				...
				if (changing_count != 1) {
					logTest<TreeModelNoInteractions>("More or less than one node has been changed in a jump!");
				}
			}
			logTest<TreeModelNoInteractions>("Skeleton is ok!");
		}
		return skeleton;
	}
	...
	TransitionRepository<Real_t> convert_skeleton_to_transition_repository(const std::vector<std::pair<State, Real_t>> &skeleton, const Real_t t_max) const {
		...
		if (TEST) {
			if (all_states.size() > skeleton.size()) {
				logTest<TreeModelNoInteractions>("There are more states in <all_states> than in the skeleton!");
			}
			std::unordered_set<State, StateHash> all_states_set(all_states.begin(), all_states.end());
			if (all_states.size() != all_states_set.size()) {
				logTest<TreeModelNoInteractions>("There are duplicate states in <all_states>!");
			}
			...
			if (std::abs(total_transition_count - skeleton.size() + 1.0) > 0.5) {
				logTest<TreeModelNoInteractions>("There are more transition counts than states!");
			}
			...
			if (total_transitions != transition_to_count.size()) {
				logTest<TreeModelNoInteractions>("Transition counts do not match!");
			}
			if (zero_transition_per_state > 1) {
				logTest<TreeModelNoInteractions>("There are more than one state with zero transitions!");
			}
		}
...
public:
	TreeModelNoInteractions<Real_t>(size_t number_of_nodes, long seed) :
		generator{ seed },
		random_tree{ seed } {
	...
	ModelData<Real_t> sample_chain(Real_t t_max) {
		tree = random_tree.generate_random_binary_tree(preferences.size());
...
#endif // !CORRELATED_MODEL_NO_INTERACTIONS_H
